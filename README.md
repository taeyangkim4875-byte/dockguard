...
```bash
dockguard scan                      # 진단 — 점수와 취약 항목을 한눈에
...
> **개발 현황** — Phase 2 완료: daemon.json 룰 10종 + 안전한 자동 수정 기능. compose · 네트워크 의존성 점검은 [로드맵](#로드맵) 참고.
...
| daemon.json 점검 | ✅ | — | — | ✅ |
| docker-compose 파일 점검 | 실행 중 컨테이너 기준 | — | — | ✅ *(Phase 3)* |
...
### Compose — docker-compose 파일 (12종, Phase 3 예정)

privileged 모드 · root 실행 · no-new-privileges · **docker.sock 마운트** · **평문 시크릿** · 읽기 전용 루트 FS · 리소스 제한 ·
host 네트워크 · 호스트 PID/IPC 공유 · 위험 capability · `latest` 태그 · 특권 포트 매핑
...
8. **compose 파일은 자동 수정하지 않습니다.** 서비스 간 의존성이 복잡해 위험하므로 수정 예시만 제시합니다 *(Phase 3)*.
...
│   ├── collector.py           # daemon.json 수집 (플랫폼별 탐색, rootless 감지, 파일 권한)
...
- **테스트 261개, 커버리지 97%** — 실제 Docker 없이 실행됩니다.
...
- [ ] **Phase 3** — docker-compose 스캐너 (룰 12종, 자동 탐색, 수정 예시 diff)
